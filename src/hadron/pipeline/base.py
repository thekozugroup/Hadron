...
from hadron import __version__, constants, envs
from hadron.distiset import create_distiset
from hadron.errors import HadronUserError
from hadron.mixins.requirements import RequirementsMixin
...
from hadron.steps.base import GeneratorStep, _Step
from hadron.steps.generators.utils import make_generator_step
from hadron.utils.logging import setup_logging, stop_logging
from hadron.utils.notebook import in_notebook
from hadron.utils.serialization import (
    _Serializable,
    read_json,
)
from hadron.utils.typing_ import (
...
if TYPE_CHECKING:
    ...
    from hadron.pipeline.routing_batch_function import RoutingBatchFunction
    from hadron.steps.base import Step
    from hadron.typing import (
        InputDataset,
        LoadGroups,
...
class BasePipeline(ABC, RequirementsMixin, _Serializable):
    """Base class for a `hadron` pipeline.
    ...
    def __init__(
        ...
        Args:
            ...
            enable_metadata: Whether to include the hadron metadata column for the pipeline
                in the final `Distiset`. It contains metadata used by hadron, for example
        ...
        elif env_cache_dir := envs.HADRON_CACHE_DIR:
            self._cache_dir = Path(env_cache_dir)
        else:
            self._cache_dir = constants.PIPELINES_CACHE_DIR

        self._logger = logging.getLogger("hadron.pipeline")
    ...
    def _validate_load_groups(self, load_groups: List[List[Any]]) -> None:  # noqa: C901
        ...
        Raises:
            HadronUserError: if something is not OK when checking the load groups.
        """
