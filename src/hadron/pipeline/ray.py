...
from hadron.constants import INPUT_QUEUE_ATTR_NAME, STEP_ATTR_NAME
...
from hadron.pipeline.base import BasePipeline, set_pipeline_running_env_variables
from hadron.pipeline.step_wrapper import _StepWrapper
...
if TYPE_CHECKING:
    ...
    from hadron.steps.base import _Step
    from hadron.typing import InputDataset, LoadGroups
...
class RayPipeline(BasePipeline):
    ...
    def run(
        ...
        self._log_queue = self.QueueClass(
            actor_options={"name": f"hadron-{self.name}-log-queue"}
        )
        ...
        self._output_queue = self.QueueClass(
            actor_options={"name": f"hadron-{self.name}-output-queue"}
        )
        self._load_queue = self.QueueClass(
            actor_options={"name": f"hadron-{self.name}-load-queue"}
        )
    ...
    def _init_ray(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "ray is not installed. Please install it using `pip install 'hadron[ray]'`."
            ) from ie
    ...
    def _create_step_input_queue(self, step_name: str) -> "Queue[Any]":
        ...
        input_queue = self.QueueClass(
            actor_options={"name": f"hadron-{self.name}-input-queue-{step_name}"}
        )
    ...
    def _run_step(self, step: "_Step", input_queue: "Queue[Any]", replica: int) -> None:
        ...
        resources: Dict[str, Any] = {
            "name": f"hadron-{self.name}-{step.name}-{replica}"
        }
