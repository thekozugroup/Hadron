...
from hadron.errors import HadronTypeError, HadronUserError
from hadron.mixins.requirements import RequirementsMixin
from hadron.mixins.runtime_parameters import (
    RuntimeParameter,
    RuntimeParametersMixin,
)
from hadron.mixins.signature import SignatureMixin
from hadron.utils.serialization import _Serializable, write_json
from hadron.utils.typing_ import is_parameter_annotated_with

if TYPE_CHECKING:
    from logging import Logger

    from hadron.pipeline.base import BasePipeline
...
_STEP_INPUT_ANNOTATION = "hadron_step_input"
...
extra metadata that allows `hadron` to perform validations over the `process` step
...
class _Step(
    ...
    the output of the previous steps. In order to make `hadron` know where the outputs
    ...
    def model_post_init(self, __context: Any) -> None:
        from hadron.pipeline.base import _GlobalPipelineManager
        ...
        if self.pipeline is None:
            _logger = logging.getLogger(f"hadron.step.{self.name}")
            _logger.warning(
    ...
    def load(self) -> None:
        ...
        """
        self._logger = logging.getLogger(f"hadron.step.{self.name}")
