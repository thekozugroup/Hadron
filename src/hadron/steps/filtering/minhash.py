...
class MinHashDedup(Step):
    ...
    Examples:
        ...
        from hadron.steps import MinHashDedup
    ...
    def load(self) -> None:
        ...
        if not importlib.import_module("datasketch"):
            raise ImportError(
                ...
                "Please install it using `pip install 'hadron[minhash]'`."
            )
        from datasketch import MinHash

        from hadron.steps.filtering._datasketch import MinHashLSH
        ...
        if self.tokenizer == "words":
            if not importlib.import_module("nltk"):
                raise ImportError(
                    ...
                    "Please install it using `pip install 'hadron[minhash]'`. Then run `nltk.download('punkt_tab')`."
                )
