...
def merge_hadron_metadata(
    *output_dicts: Dict[str, Any],
) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
    """
    Merge the `HADRON_METADATA_KEY` from multiple output dictionaries. `HADRON_METADATA_KEY`
    can be either a dictionary containing metadata keys or a list containing dictionaries
    of metadata keys.

    Args:
        *output_dicts: Variable number of dictionaries or lists containing hadron metadata.

    Returns:
        A merged dictionary or list containing all the hadron metadata.
    """
    merged_metadata = defaultdict(list)

    for output_dict in output_dicts:
        metadata = output_dict.get(HADRON_METADATA_KEY, {})
        # If `hadron_metadata_key` is a `list` then it contains dictionaries with
    ...
    *inputs: "StepInput",
    ...
    for dicts_at_index in zip(*inputs):
        ...
        for d in dicts_at_index:
            ...
            if HADRON_METADATA_KEY in d:
                metadata_dicts.append(
                    {HADRON_METADATA_KEY: d[HADRON_METADATA_KEY]}
                )
            # Iterate over key-value pairs in each dict
            for key, value in d.items():
                if key == HADRON_METADATA_KEY:
                    continue
        ...
        if metadata_dicts:
            combined_dict[HADRON_METADATA_KEY] = merge_hadron_metadata(
                *metadata_dicts
            )
