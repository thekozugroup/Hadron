...
"""Hadron Task integration for the AutoReason tournament.

This module exposes :class:`AutoReasonedGeneration`, a
:class:`hadron.steps.tasks.base.Task` that runs an AutoReason tournament
...
:class:`~hadron.steps.tasks.autoreason.tournament.TournamentRunner`. To
...
from hadron.steps.tasks.autoreason.rate_limit import get_limiter
from hadron.steps.tasks.autoreason.tournament import TournamentRunner
from hadron.steps.tasks.base import Task
from hadron.utils.template import check_column_in_template

if TYPE_CHECKING:
    from hadron.steps.base import StepInput
    from hadron.typing import ChatType, StepColumns, StepOutput

# Hadron depends on ``nest_asyncio`` and applies it when running inside a
...
# running loops. This mirrors ``hadron.models.llms.base``.
...
class AutoReasonedGeneration(Task):
    ...
    :class:`~hadron.steps.tasks.autoreason.tournament.TournamentRunner`
    ...
    Examples:
        ...
        ```python
        from hadron.steps.tasks import AutoReasonedGeneration
        from hadron.models import OpenAILLM
    ...
    def _row_from_trace(self, row: Dict[str, Any], trace) -> Dict[str, Any]:
        ...
        # Per-framework convention, stash metadata under HADRON_METADATA_KEY.
        meta = out.get(HADRON_METADATA_KEY, {}) or {}
        meta[f"statistics_{self.name}"] = {
            "total_calls": trace.total_calls,
            "winner_source": trace.winner_source,
        }
        out[HADRON_METADATA_KEY] = meta
        return out
