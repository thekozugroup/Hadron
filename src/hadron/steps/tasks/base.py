...
from hadron.models.image_generation.base import ImageGenerationModel
...
from hadron.utils.dicts import group_dicts
...
class _Task(_Step, ABC):
    ...
    Attributes:
        ...
        add_raw_output: whether to include a field with the raw output of the LLM in the
            `hadron_metadata` field of the output. Can be helpful to not loose data
    ...
    add_raw_output: RuntimeParameter[bool] = Field(
        ...
        description=(
            ...
            " of the `hadron_metadata` dictionary output column"
        ),
    )
    add_raw_input: RuntimeParameter[bool] = Field(
        ...
        description=(
            ...
            " of the `hadron_metadata` dictionary column"
        ),
    )
    ...
    def _format_outputs(
        ...
        for (output, stats, extra), input in zip(
            ...
            try:
                # Extract the generations, and move the statistics to the hadron_metadata,
    ...
    def _output_on_failure(
        ...
        a new field `hadron_meta` with the raw output of the LLM.
        """
    ...
    def _create_metadata(
        ...
        """
        meta = output.get(HADRON_METADATA_KEY, {})
        ...
        if meta:
            output[HADRON_METADATA_KEY] = meta
    ...
    def _set_default_structured_output(self) -> None:
        ...
        if self.use_default_structured_output and not self.llm.structured_output:
            ...
            # the LLM is loaded
            from hadron.models.llms import InferenceEndpointsLLM
    ...
    def print(self, sample_input: Optional["ChatType"] = None) -> None:
        ...
        Examples:
            ...
            ```python
            from hadron.steps.tasks import URIAL
            from hadron.models.llms.huggingface import InferenceEndpointsLLM
