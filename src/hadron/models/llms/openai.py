...
from hadron.models.llms.utils import prepare_output
...
if TYPE_CHECKING:
    ...
    from hadron.typing.models import (
        LLMStatistics,
        Logprob,
        StandardInput,
        StructuredInput,
...
class OpenAILLM(OpenAIBaseClient, AsyncLLM):
    ...
    Attributes:
        ...
        structured_output: a dictionary containing the structured output configuration configuration
            ...
            `InstructorStructuredOutputType` from `hadron.steps.tasks.structured_outputs.instructor`.
    ...
    Examples:
        ...
        ```python
        from hadron.models.llms import OpenAILLM
    ...
    def offline_batch_generate(
        ...
        **kwargs: Any,
        ...
        Raises:
            HadronOfflineBatchGenerationNotFinishedException: if the batch generation
                is not finished yet.
    ...
    def _create_batch_api_job(
        ...
        metadata = {"description": "hadron"}

        if hadron_pipeline_name := envs.HADRON_PIPELINE_NAME:
            metadata["hadron_pipeline_name"] = hadron_pipeline_name

        if hadron_pipeline_cache_id := envs.HADRON_PIPELINE_CACHE_ID:
            metadata["hadron_pipeline_cache_id"] = hadron_pipeline_cache_id
    ...
    def _create_batch_files(
        ...
        for file_no, buffer in enumerate(
            ...
            try:
                # TODO: add hadron pipeline name and id
    ...
    def _name_for_openai_files(self, file_no: int) -> str:
        if (
            envs.HADRON_PIPELINE_NAME is None
            or envs.HADRON_PIPELINE_CACHE_ID is None
        ):
            return f"hadron-pipeline-fileno-{file_no}.jsonl"

        return f"hadron-pipeline-{envs.HADRON_PIPELINE_NAME}-{envs.HADRON_PIPELINE_CACHE_ID}-fileno-{file_no}.jsonl"
