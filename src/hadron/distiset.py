...
from hadron.utils.card.dataset_card import (
    HadronDatasetCard,
    size_categories_parser,
)
from hadron.utils.docstring import get_bibtex, parse_google_docstring
from hadron.utils.files import list_files_in_dir
from hadron.utils.huggingface import get_hf_token
...
class Distiset(dict):
    ...
    def _get_card(
        ...
        metadata = {
            **readme_metadata,
            ...
            "tags": ["synthetic", "hadron", "rlaif"],
        }

        card = HadronDatasetCard.from_template(
    ...
    def transform_columns_to_image(self, columns: Union[str, list[str]]) -> Self:
        ...
        from hadron.models.image_generation.utils import image_from_str
...
def create_distiset(  # noqa: C901
    ...
    Args:
        ...
        enable_metadata: Whether to include the hadron metadata column in the dataset or not.
            Defaults to `False`.
    ...
    Examples:
        ```python
        from pathlib import Path
        distiset = create_distiset(Path.home() / ".cache/hadron/pipelines/path-to-pipe-hashname")
        ```
    """
    from hadron.constants import HADRON_METADATA_KEY

    logger = logging.getLogger("hadron.distiset")
    ...
    for file in steps_outputs_dir.iterdir():
        ...
        if files:
            try:
                ...
                if not enable_metadata and HADRON_METADATA_KEY in ds.column_names:
                    ds = ds.remove_columns(HADRON_METADATA_KEY)
