...
from hadron.pipeline.templates.base import BasePipelineTemplate
from hadron.steps.base import _Step
from hadron.steps.tasks.base import _Task
...
class ComponentsInfo(TypedDict):
    """A dictionary containing `hadron` components information."""
...
def export_components_info() -> ComponentsInfo:
    """Exports `hadron` components (`LLM`s, `Step`s and `Task`s) information in a dictionary
    format. This information can be used to generate `hadron` components documentation,
...
def _get_image_generation_models() -> List[Type["ImageGenerationModel"]]:
    ...
    Returns:
        The list of all the classes under `hadron.models.image_generation` that are not abstract classes.
    """
    return [
        igm_type
