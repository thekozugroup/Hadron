...
from distilagent.utils.logging import setup_logging, stop_logging
...
class RayPipeline(BasePipeline):
    ...
    def __init__(
        ...
        Args:
            ...
            enable_metadata: Whether to include the distilagent metadata column for the pipeline
                in the final `Distiset`. It contains metadata used by distilagent, for example
    ...
    def run(
        ...
        self._log_queue = self.QueueClass(
            actor_options={"name": f"distilagent-{self.name}-log-queue"}
        )
        ...
        self._output_queue = self.QueueClass(
            actor_options={"name": f"distilagent-{self.name}-output-queue"}
        )
        self._load_queue = self.QueueClass(
            actor_options={"name": f"distilagent-{self.name}-load-queue"}
        )
    ...
    def _init_ray(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "ray is not installed. Please install it using `pip install 'distilagent[ray]'`."
            ) from ie
    ...
    def _create_step_input_queue(self, step_name: str) -> "Queue[Any]":
        ...
        input_queue = self.QueueClass(
            actor_options={"name": f"distilagent-{self.name}-input-queue-{step_name}"}
        )
    ...
    def _run_step(self, step: "_Step", input_queue: "Queue[Any]", replica: int) -> None:
        ...
        resources: Dict[str, Any] = {
            "name": f"distilagent-{self.name}-{step.name}-{replica}"
        }
