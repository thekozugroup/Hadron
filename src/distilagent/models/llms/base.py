...
class LLM(RuntimeParametersModelMixin, BaseModel, _Serializable, ABC):
    ...
    def load(self) -> None:
        ...
        self._logger = logging.getLogger(f"distilagent.llm.{self.model_name}")
...
class AsyncLLM(LLM):
    ...
    def _prepare_structured_output(  # type: ignore
        ...
        """
        from distilagent.steps.tasks.structured_outputs.instructor import (
            prepare_instructor,
        )
    ...
    def _prepare_kwargs(
        ...
        if not (inspect.isclass(schema) and issubclass(schema, BaseModel)):
            from distilagent.steps.tasks.structured_outputs.utils import (
