...
from distilagent.models.image_generation.base import ImageGenerationModel
...
from distilagent.utils.dicts import group_dicts
...
class _Task(_Step, ABC):
    ...
    Attributes:
        ...
        add_raw_output: whether to include a field with the raw output of the LLM in the
            `distilagent_metadata` field of the output. Can be helpful to not loose data
    ...
    add_raw_output: RuntimeParameter[bool] = Field(
        ...
        description=(
            ...
            " of the `distilagent_metadata` dictionary output column"
        ),
    )
    add_raw_input: RuntimeParameter[bool] = Field(
        ...
        description=(
            ...
            " of the `distilagent_metadata` dictionary column"
        ),
    )
    ...
    def _format_outputs(
        ...
        for (output, stats, extra), input in zip(
            ...
            try:
                # Extract the generations, and move the statistics to the distilagent_metadata,
    ...
    def _output_on_failure(
        ...
        a new field `distilagent_meta` with the raw output of the LLM.
        """
    ...
    def _create_metadata(
        ...
        """
        meta = output.get(DISTILAGENT_METADATA_KEY, {})
        ...
        if meta:
            output[DISTILAGENT_METADATA_KEY] = meta
    ...
    def _set_default_structured_output(self) -> None:
        ...
        if self.use_default_structured_output and not self.llm.structured_output:
            ...
            # the LLM is loaded
            from distilagent.models.llms import InferenceEndpointsLLM
    ...
    def print(self, sample_input: Optional["ChatType"] = None) -> None:
        ...
        Examples:
            ...
            ```python
            from distilagent.steps.tasks import URIAL
            from distilagent.models.llms.huggingface import InferenceEndpointsLLM
