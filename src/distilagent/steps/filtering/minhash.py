...
class MinHashDedup(Step):
    ...
    Examples:
        ...
        from distilagent.steps import MinHashDedup
    ...
    def load(self) -> None:
        ...
        if not importlib.import_module("datasketch"):
            raise ImportError(
                ...
                "Please install it using `pip install 'distilagent[minhash]'`."
            )
        from datasketch import MinHash

        from distilagent.steps.filtering._datasketch import MinHashLSH
        ...
        if self.tokenizer == "words":
            if not importlib.import_module("nltk"):
                raise ImportError(
                    ...
                    "Please install it using `pip install 'distilagent[minhash]'`. Then run `nltk.download('punkt_tab')`."
                )
