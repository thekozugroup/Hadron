...
from distilagent.mixins.signature import SignatureMixin
from distilagent.utils.serialization import _Serializable, write_json
...
class _Step(
    ...
    the output of the previous steps. In order to make `distilagent` know where the outputs
    ...
    def model_post_init(self, __context: Any) -> None:
        from distilagent.pipeline.base import _GlobalPipelineManager
        ...
        if self.pipeline is None:
            _logger = logging.getLogger(f"distilagent.step.{self.name}")
            _logger.warning(
    ...
    def load(self) -> None:
        ...
        """
        self._logger = logging.getLogger(f"distilagent.step.{self.name}")
